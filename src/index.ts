...
async function GetAircraft() {
  ...
  await axios.get('https://data.vatsim.net/v3/vatsim-data.json').then(async (resp) => {
    let networkPilots: any = [];
    const flightMapPilots: Pilot[] = [];

    const response = resp.data;
    networkPilots = response.pilots;

    networkPilots = networkPilots.filter((pilot: any) => {
    ...
    nonBusyRegions = Shuffle(nonBusyRegions).slice(0, 50);
    busyRegions = Shuffle(busyRegions).slice(0, 100 - nonBusyRegions.length);
...
async function ComputeFlightMapData(pilot: any) {
...
async function GetAirportData(icao: string) : Promise<Airport> {
  const [results, fields] = await db.promise().query('SELECT * FROM `airports` WHERE `icao` = ? limit 0, 1', [icao]);
